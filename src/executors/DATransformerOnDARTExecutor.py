...
@register_executor
class DATransformerOnDARTExecutor(BaseExecutor):
    def __init__(self,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        triples = [t for t in self.val_triplesets[batch_idx*bsize:(batch_idx+1)*bsize]]
        decoded_tokens_dict.update({'triples': triples})
    ...
    def setup(self, stage):
        ...
        if stage in (None, "fit"):
            ...
            self.val_triplesets = data['validation']['tripleset']
        # Assign Test split(s) for use in Dataloaders
        if stage in (None, "test"):
            self.whole_test_dataset = copy.deepcopy(data['test'])
            self.test_triplesets = data['test']['tripleset']
    ...
    def test_step(self, batch, batch_idx):
        x, y, mask = batch['input_ids'], batch['labels'], batch['attention_mask'] #NOTE: only works for batch_size = 1
        x, mask, y = batch_depad(x, mask, y, pad_len=1)
        ...
        decoded_tokens_dict = self._decode_generative_step(x, y, forced_token_ids=forced_token_ids, specified_length_constraint=specified_length_constraint)

        bsize = x.shape[0]
        triples = [t for t in self.test_triplesets[batch_idx*bsize:(batch_idx+1)*bsize]]
        decoded_tokens_dict.update({'triples': triples})
        log_dict = decoded_tokens_dict
    ...
    def _decode_generative_step(self, x, y=None, forced_token_ids=None, specified_length_constraint=None, inspect_dag=True):
        ...
        inps = self.tokenizer.batch_decode(x, skip_special_tokens=False)
        
        if forced_token_ids:
            # breakpoint()
            forced_words = [self.tokenizer.batch_decode(token_ids, skip_special_tokens=True) for token_ids in forced_token_ids]
            print("forced_words", forced_words)
        print("inps:", inps)
        print("refs:", refs)
        print("preds:", preds)
        print("="*100)
